...
function setSidebarCookie(value: boolean) {
  if (typeof window !== "undefined" && document?.cookie !== undefined) {
    try {
      const cookieValue = `${SIDEBAR_COOKIE_NAME}=${value}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}; SameSite=Lax`;

      const tempDocument = document as Document & { cookie: string };
      tempDocument.cookie = cookieValue;
    } catch (error) {
      // biome-ignore lint/suspicious/noConsole: Ignore console.warn in this context
